...
public class ControleMenuOp {
    ...
    public void afficherProduit (){ //je ne sais pas si les méthodes je peux les nommer pareil que dans MenuChef, pour l'instant je laisse comme ca 
        ...
        nouvelle_vueA.actionProd(view3.getScene()); //il faudra créer la méthode afficheProduit dans la classe ProduitView mais je n'ai pas osé y toucher pour l'instant
        }
    
    public void afficherGamme () {
        GammeView nouvelle_vueB = new GammeView(); //bizarre car ca marche ici
        nouvelle_vueB.afficheGamme(view3.getScene()); // Pareil je ne sais pas si cette méthode existe, j'adapterai le nom en fonction de ce qui a été mis dans GammeView
        }
   ...
   public void afficherOperation (){
       ...
       nouvelle_vueC.actionOp(view3.getScene()); //pareil il faudra que j'aille faire la méthode
   }
